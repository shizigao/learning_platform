...
erDiagram
    ...
    USER ||--o{ LEARNING_CLASS : owns
    LEARNING_CLASS ||--o{ CLASS_MEMBER : contains
    USER ||--o{ CLASS_MEMBER : joins
    LEARNING_CLASS ||--o{ CLASS_ANNOUNCEMENT : publishes
    LEARNING_CLASS ||--o{ CONTENT_CLASS_SCOPE : receives
    LEARNING_CONTENT ||--o{ CONTENT_CLASS_SCOPE : distributed_to
    LEARNING_CLASS ||--o{ EXAM_CLASS_SCOPE : receives
    EXAM ||--o{ EXAM_CLASS_SCOPE : assigned_to
...
- `learning_content.content_type`：统一为 `GENERAL`。
- `learning_content.distribution_mode`：`PUBLIC`、`CLASS`。
- `content_file.file_role`：`COVER`、`INLINE_IMAGE`、`CONTENT`、`VIDEO`、`ATTACHMENT`、`SUBTITLE`。
...
- `exam.assignment_mode`：`INDIVIDUAL`、`CLASS`。
