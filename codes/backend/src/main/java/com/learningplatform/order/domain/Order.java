/* 文件职责：表示订单领域对象或组件，封装该概念相关的数据和行为。
 * 所属模块：商品、订单、支付模拟与用户权益；所在分层：领域模型层。
 * 维护提示：修改本文件时应同步检查相关 DTO、Mapper、Service、Controller 与测试。
 */
package com.learningplatform.order.domain;

import com.learningplatform.common.model.BaseEntity;

import java.math.BigDecimal;
import java.time.LocalDateTime;

/**
 * 表示订单领域对象或组件，封装该概念相关的数据和行为。
 *
 * <p>职责边界：保存领域状态，不依赖 Web 层，也不负责发起外部调用。</p>
 */
public class Order extends BaseEntity {
    /** 保存订单No，供该类型的业务逻辑读取或更新。 */
    private String orderNo;
    /** 保存用户ID，供该类型的业务逻辑读取或更新。 */
    private Long userId;
    /** 保存状态，供该类型的业务逻辑读取或更新。 */
    private OrderStatus status;
    /** 保存总计Amount，供该类型的业务逻辑读取或更新。 */
    private BigDecimal totalAmount;
    /** 保存payableAmount，供该类型的业务逻辑读取或更新。 */
    private BigDecimal payableAmount;
    /** 保存paidAmount，供该类型的业务逻辑读取或更新。 */
    private BigDecimal paidAmount;
    /** 保存支付Method，供该类型的业务逻辑读取或更新。 */
    private String paymentMethod;
    /** 保存remark，供该类型的业务逻辑读取或更新。 */
    private String remark;
    /** 保存过期时间，供该类型的业务逻辑读取或更新。 */
    private LocalDateTime expiresAt;
    /** 保存paid时间，供该类型的业务逻辑读取或更新。 */
    private LocalDateTime paidAt;
    /** 保存cancelled时间，供该类型的业务逻辑读取或更新。 */
    private LocalDateTime cancelledAt;
    /** 保存version，供该类型的业务逻辑读取或更新。 */
    private Integer version;

    /** 返回订单No。 */
    public String getOrderNo() {
        return orderNo;
    }

    /** 更新订单No；调用方仍需遵守所属领域的校验规则。 */
    public void setOrderNo(String orderNo) {
        this.orderNo = orderNo;
    }

    /** 返回用户ID。 */
    public Long getUserId() {
        return userId;
    }

    /** 更新用户ID；调用方仍需遵守所属领域的校验规则。 */
    public void setUserId(Long userId) {
        this.userId = userId;
    }

    /** 返回状态。 */
    public OrderStatus getStatus() {
        return status;
    }

    /** 更新状态；调用方仍需遵守所属领域的校验规则。 */
    public void setStatus(OrderStatus status) {
        this.status = status;
    }

    /** 返回总计Amount。 */
    public BigDecimal getTotalAmount() {
        return totalAmount;
    }

    /** 更新总计Amount；调用方仍需遵守所属领域的校验规则。 */
    public void setTotalAmount(BigDecimal totalAmount) {
        this.totalAmount = totalAmount;
    }

    /** 返回PayableAmount。 */
    public BigDecimal getPayableAmount() {
        return payableAmount;
    }

    /** 更新PayableAmount；调用方仍需遵守所属领域的校验规则。 */
    public void setPayableAmount(BigDecimal payableAmount) {
        this.payableAmount = payableAmount;
    }

    /** 返回PaidAmount。 */
    public BigDecimal getPaidAmount() {
        return paidAmount;
    }

    /** 更新PaidAmount；调用方仍需遵守所属领域的校验规则。 */
    public void setPaidAmount(BigDecimal paidAmount) {
        this.paidAmount = paidAmount;
    }

    /** 返回支付Method。 */
    public String getPaymentMethod() {
        return paymentMethod;
    }

    /** 更新支付Method；调用方仍需遵守所属领域的校验规则。 */
    public void setPaymentMethod(String paymentMethod) {
        this.paymentMethod = paymentMethod;
    }

    /** 返回Remark。 */
    public String getRemark() {
        return remark;
    }

    /** 更新Remark；调用方仍需遵守所属领域的校验规则。 */
    public void setRemark(String remark) {
        this.remark = remark;
    }

    /** 返回过期时间。 */
    public LocalDateTime getExpiresAt() {
        return expiresAt;
    }

    /** 更新过期时间；调用方仍需遵守所属领域的校验规则。 */
    public void setExpiresAt(LocalDateTime expiresAt) {
        this.expiresAt = expiresAt;
    }

    /** 返回Paid时间。 */
    public LocalDateTime getPaidAt() {
        return paidAt;
    }

    /** 更新Paid时间；调用方仍需遵守所属领域的校验规则。 */
    public void setPaidAt(LocalDateTime paidAt) {
        this.paidAt = paidAt;
    }

    /** 返回Cancelled时间。 */
    public LocalDateTime getCancelledAt() {
        return cancelledAt;
    }

    /** 更新Cancelled时间；调用方仍需遵守所属领域的校验规则。 */
    public void setCancelledAt(LocalDateTime cancelledAt) {
        this.cancelledAt = cancelledAt;
    }

    /** 返回Version。 */
    public Integer getVersion() {
        return version;
    }

    /** 更新Version；调用方仍需遵守所属领域的校验规则。 */
    public void setVersion(Integer version) {
        this.version = version;
    }
}
