...
@SpringBootTest(properties = {
        "app.ai.mock.scenario=timeout",
        "app.ai.mock.delay=500ms",
        "app.ai.limits.timeout=30ms"
})
@AutoConfigureMockMvc
@ActiveProfiles("test")
@Sql({"/sql/user-schema.sql", "/sql/content-schema.sql", "/sql/ai-schema.sql"})
class MockAiTimeoutIntegrationTests {
    ...
    void setUp() throws Exception {
        ...
        contentId = insertContent(user.getId());
        token = login();
    }

    @Test
    void timeoutCreatesFailedTaskWithoutResultUsageOrDeduction() throws Exception {
        mockMvc.perform(post("/api/ai/contents/{contentId}/summaries", contentId)
                ...
                .andExpect(jsonPath("$.message").value("AI 请求处理超时，请稍后重试"));

        assertThat(quota()).isEqualTo(5);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT status FROM ai_task WHERE request_id = 'mock-timeout'",
                String.class
        )).isEqualTo("FAILED");
        assertThat(jdbcTemplate.queryForObject(
                "SELECT error_code FROM ai_task WHERE request_id = 'mock-timeout'",
                String.class
        )).isEqualTo("TIMEOUT");
        assertThat(jdbcTemplate.queryForObject(
    ...
    private User createUser() {
        ...
        created.setUsername("mock_timeout_user");
        created.setPasswordHash(passwordEncoder.encode(PASSWORD));
        created.setNickname("mock_timeout_user");
    ...
    private long insertContent(Long publisherId) {
        jdbcTemplate.update(
                """
                ...
                ) VALUES (?, 1, 'Mock 超时测试资料', '测试简介', 'ARTICLE',
                          '测试正文', TRUE, 0.00, 'PUBLISHED', CURRENT_TIMESTAMP)
                """,
                publisherId
        );
        return jdbcTemplate.queryForObject(
                "SELECT id FROM learning_content WHERE title = 'Mock 超时测试资料'",
                Long.class
        );
    }
    ...
    private String login() throws Exception {
        MvcResult result = mockMvc.perform(post("/api/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"username":"mock_timeout_user","password":"Password123"}
                                """))
